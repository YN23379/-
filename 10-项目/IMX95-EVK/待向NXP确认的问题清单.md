...
## ✅ 2026-09-21 已确认：Pro 板不在 Harpoon 支持范围，决定换 EVK

**NXP 侧（Dongbo Wang）的回复原文**：

> "Jailhouse 那一套看起来支持的是 imx95evk 板子" / "跟你这个不是一个板子" /
> "板级适配要修改很多" / "你们只有这一个板子吗？" /
> "后续是准备基于 Imx95frdm-pro 做设计和开发吗？还是要基于 evk？" /
> "这个修改动作可能有点大，我要评估一下。"

**我方回复**：只有 Pro 板一块，**决定改用 i.MX95 EVK**。

**对方确认型号**：**i.MX95 19x19 EVK**（即 `imx95-19x19-lpddr5-evk`），
**明天邮寄给我方**，我方把 Pro 板交换过去。

### 这条确认意味着什么

| 之前的判断 | 现在的状态 |
|---|---|
| Harpoon 包是 EVK 版、Pro 板不在支持矩阵里 | **官方确认**（升级自"推测"） |
| 想跑通必须做板级适配（DTB、SM 配置、cell、脚本机器判断） | **官方确认**："板级适配要修改很多" |
| inmate console 无输出（LPUART3 不归 Linux 域） | **与"板级适配"一致**，换到 EVK 后预期自然消失 |
| 换板后包能不能直接用 | EVK 就是包的对应板（`HS_v3.5_IMX95-19X19-LPDDR5-EVK`），**预期可以直接用** |

> **换板后不再需要**：手工设 `jh_root_mem` / `jh_clk` 绕过（EVK 有官方的 `imx95-19x19-evk-harpoon.dtb`，
> 直接 `setenv jh_root_dtb ...` + `run jh_mmcboot` 即可）。

### 下面这些内容的历史价值

下面的问题 1 是我们**在对方回复之前**做的排查记录。**结论已被官方确认**，
保留它是为了留下完整证据链（从"实测推断"到"官方确认"的过程）。
**换板后如果 console 正常，这一节就可以整体收进归档。**
