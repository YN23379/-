...
| 文件 | 内容 |
|---|---|
| **`Harpoon复现.md`** | **Harpoon 手把手操作手册 + 原理**（76 KB，本目录最完整的一篇；换板后流程通用，只需把手工设内存改成 EVK 的官方方式） |
| **`上电流程.md`** | **上电全过程看什么**：分 14 个阶段，每段在哪个串口看、应该看到什么、怎么判断正常；含排查表和原始日志行号 |
| `理解-i.MX95启动与资源隔离.md` | 启动逻辑、资源隔离与权限（**换板后仍然适用**） |
...
**建议阅读顺序**：想快速了解这块板上做过什么 → 先看 `Harpoon复现.md`；
想知道板子怎么启动、卡住了怎么查 → 看 `上电流程.md`。
